...
//     //for all templated structs add them
//     std::cout << "ADDING TEMPLATED STRUCTS" << std::endl;
//     for(int i = 0; i < templated_structs.size(); i++) {
//         TemplatedStructDefinition *ts = templated_structs[i];
//         if(ts->header->types.size() == 0) continue;
//         StructDefinition *s = ts->struct_def;
//         BaseType *sbt = dynamic_cast<BaseType*>(s->type);
//         if(!add_templated_struct(templated_structs[i])) {
//             std::cout << "Failed to add struct : " << sbt->to_string() << "\n";
//             return false;
//         }
//     }

//     //for all non-templated structs add them
...
//     // - are all non-templated structs well formed?
...
//     // - can we construct a struct layout for all non-templated structs?
//     std::cout << "CONSTRUCT BASE STRUCT LAYOUTS" << std::endl;
//     for(int i = 0; i < templated_structs.size(); i++){
//         TemplatedStructDefinition *ts = templated_structs[i];
//         if(ts->header->types.size() != 0) continue;
//         StructDefinition *s = ts->struct_def;
//         if(!construct_struct_layout(s->type)) {
//             std::cout << "Unable to construct struct layout for type : " << s->type->to_string() << "\n";
//             return false;
//         }
//     }
...
//     // - check all methods, make sure they're well formed
//     int function_ptr = 0;
